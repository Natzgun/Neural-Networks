...
// hacer gradient checking numerico sobre dX de todo el bloque.
float loss_fn(TransformerBlock& block, const Tensor& x, const Tensor& dy) {
...
int main() {
  ...
  int embed_dim = 8;
  int heads = 2;
  int mlp_hidden_dim = 16;

  TransformerBlock block(embed_dim, heads, mlp_hidden_dim);
  ...
  // Gradient checking numerico: confirma que las dos ramas residuales
  // (attention y MLP) estan bien encadenadas de punta a punta. Debe correr
  // ANTES de update(), porque update() cambiaria los pesos y el dX analitico
  // quedaria comparando contra una funcion distinta a la que se evaluo.
  X.download();
  ...
  for (int b = 0; b < batch; ++b) {
    for (int n = 0; n < tokens; ++n) {
      for (int e = 0; e < embed_dim; ++e) {
        ...
        float loss_plus = loss_fn(block, x_plus, dY);
        float loss_minus = loss_fn(block, x_minus, dY);
  ...
  // update() no debe crashear (ejercita las 6 sub-capas con parametros). Va
  // al final para no invalidar el gradient checking de arriba.
  block.update(0.01f);
  std::cout << "update: OK\n";

  std::cout << "TransformerBlock: shapes, update y gradient checking correctos." << std::endl;
  return 0;
}
