...
Tensor::Tensor(const Tensor& other)
  ...
  if (other.d_data_) {
    ...
    CUDA_CHECK(
        cudaMemcpy(d_data_, other.d_data_, numel() * sizeof(float), cudaMemcpyDeviceToDevice));
  }
}
...
Tensor::Tensor(Tensor&& other) noexcept
    : data_(std::move(other.data_)), d_data_(other.d_data_), shape_(std::move(other.shape_)),
...
const std::vector<int>& Tensor::sizes() const {
  return shape_;
}

Tensor Tensor::reshape(const std::vector<int>& new_shape) const {
  int new_numel = std::accumulate(new_shape.begin(), new_shape.end(), 1, std::multiplies<int>());
  if (new_numel != numel()) {
    std::cerr << "reshape: shape mismatch (" << new_numel << " vs " << numel() << ")" << std::endl;
  ...
  if (d_data_) {
    ...
    CUDA_CHECK(cudaMemcpy(out.d_data_, d_data_, numel() * sizeof(float), cudaMemcpyDeviceToDevice));
  }
  return out;
}
...
void Tensor::upload() {
  ...
  CUDA_CHECK(cudaMemcpy(d_data_, data_.data(), numel() * sizeof(float), cudaMemcpyHostToDevice));
}
...
void Tensor::download() const {
  if (!d_data_)
    return;
  data_.resize(numel());
  CUDA_CHECK(cudaMemcpy(data_.data(), d_data_, numel() * sizeof(float), cudaMemcpyDeviceToHost));
}
...
float& Tensor::flat(int i) {
  return data_[i];
}
float Tensor::flat(int i) const {
  return data_[i];
}
...
float* Tensor::device_ptr() {
  return d_data_;
}
const float* Tensor::device_ptr() const {
  return d_data_;
}
bool Tensor::on_device() const {
