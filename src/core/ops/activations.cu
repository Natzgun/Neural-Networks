...
__global__ void relu_derivative_kernel(const float* output, float* out, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) out[idx] = output[idx] > 0.0f ? 1.0f : 0.0f;
}
...
__global__ void sigmoid_derivative_kernel(const float* output, float* out, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) out[idx] = output[idx] * (1.0f - output[idx]);
}
...
Tensor relu_derivative(const Tensor& output) {
    ...
    r.upload();
    int n = output.numel();
    relu_derivative_kernel<<<div_ceil(n, 256), 256>>>(output.device_ptr(),
                                                       r.device_ptr(), n);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    return r;
...
Tensor sigmoid_derivative(const Tensor& output) {
    ...
    r.upload();
    int n = output.numel();
    sigmoid_derivative_kernel<<<div_ceil(n, 256), 256>>>(output.device_ptr(),
                                                          r.device_ptr(), n);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    return r;
