...
Tensor add(const Tensor& a, const Tensor& b) {
    ...
    r.upload();
    int n = a.numel();
    add_kernel<<<div_ceil(n, 256), 256>>>(a.device_ptr(), b.device_ptr(),
                                          r.device_ptr(), n);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    return r;
...
Tensor sub(const Tensor& a, const Tensor& b) {
    ...
    r.upload();
    int n = a.numel();
    sub_kernel<<<div_ceil(n, 256), 256>>>(a.device_ptr(), b.device_ptr(),
                                          r.device_ptr(), n);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    return r;
...
Tensor hadamard(const Tensor& a, const Tensor& b) {
    ...
    r.upload();
    int n = a.numel();
    hadamard_kernel<<<div_ceil(n, 256), 256>>>(a.device_ptr(), b.device_ptr(),
                                               r.device_ptr(), n);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    return r;
...
Tensor matmul(const Tensor& a, const Tensor& b) {
    ...
    matmul_kernel<<<grid, block>>>(a.device_ptr(), b.device_ptr(), r.device_ptr(),
                                   M, N, K);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    return r;
...
Tensor add_bias(const Tensor& a, const Tensor& bias) {
    ...
    r.upload();
    int n = a.numel();
    add_bias_kernel<<<div_ceil(n, 256), 256>>>(a.device_ptr(), bias.device_ptr(),
                                               r.device_ptr(), rows, cols);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    return r;
}
...
Tensor sum_rows(const Tensor& a) {
    ...
    sum_rows_kernel<<<div_ceil(cols, 256), 256>>>(a.device_ptr(), r.device_ptr(),
                                                  rows, cols);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
    return r;
