...
__global__ void maxpool2d_forward_kernel(const float* in, float* out, int* mask,
...
__global__ void maxpool2d_backward_kernel(const float* dout, const int* mask,
                                          float* din,
                                          int N, int C, int outH, int outW) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int total = N * C * outH * outW;
    if (idx >= total) return;
...
Tensor maxpool2d_forward(const Tensor& input, int kernel_size, int stride,
                         PoolMask& mask) {
    int N = input.dim(0);
    int C = input.dim(1);
    int H = input.dim(2);
    ...
    int total = new_size;
    maxpool2d_forward_kernel<<<div_ceil(total, 256), 256>>>(
        input.device_ptr(), output.device_ptr(), mask.data,
        N, C, H, W, kernel_size, stride, outH, outW);
...
Tensor maxpool2d_backward(const Tensor& grad_output, const PoolMask& mask,
    ...
    maxpool2d_backward_kernel<<<div_ceil(total, 256), 256>>>(
        grad_output.device_ptr(), mask.data, grad_input.device_ptr(),
        N, C, outH, outW);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
