...
Conv2DLayer::Conv2DLayer(int in_channels, int out_channels, int kernel_size, int stride,
                         int padding)
  ...
  weights_ =
      Tensor::random_normal({out_channels, in_channels, kernel_size, kernel_size}, 0.0f, stddev);
...
Tensor Conv2DLayer::backward(const Tensor& grad_output) {
  grad_weights_ = ops::conv2d_backward_weights(inputs_, grad_output, stride_, padding_,
                                               kernel_size_, out_channels_);
  grad_biases_ = ops::conv2d_backward_bias(grad_output);

  Tensor grad_input =
      ops::conv2d_backward_input(grad_output, weights_, stride_, padding_, inputs_.sizes());
